...
const BlocksEditor = ({ blocks = [], onChange }) => {
  ...
  return (
    <div>
      <label className="block text-slate-400 mb-1 font-medium">محتوى الصفحة (بلوكات)</label>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[11px] text-slate-300 font-semibold">فقرة #{index + 1}</span>
  ...
  return (
    <div className="no-print w-full lg:w-[320px] bg-slate-900/70 border border-slate-700/60 rounded-3xl p-4 flex flex-col shadow-[0_18px_45px_rgba(15,23,42,0.9)]">
      ...
      <div className="space-y-3 text-xs overflow-y-auto max-h-[520px] pr-1">
        ...
        {/* Images */}
        {onImagesChange && (
          <ImagesEditor images={page.images || []} onChange={onImagesChange} />
        )}
...
const PageContent = ({ page, pageNumber, totalPages, bookMeta, printMode = false }) => {
  ...
  const renderImages = () => {
    if (!page.images || !page.images.length) return null;
      ...
      <div className="grid grid-cols-1 gap-8 mt-8">
        {page.images.map((img, idx) => {
          ...
          if (!src) {
            return (
              <EnhancedBlueprintImage
                key={img.id || `no-src-${idx}`}
                desc={img.caption || 'صورة بدون مصدر'}
                index={idx}
                theme={theme}
              />
            );
          ...
          return (
            <div key={img.id || `img-${idx}`} className="flex flex-col items-center gap-3">
              ...
              {img.caption && (
                <p className="text-xs text-slate-500 text-center max-w-md">{img.caption}</p>
              )}
            </div>
          );
        })}
...
const BilingualBook = () => {
  ...
  const updateCurrentPageImages = (images) => {
    updateCurrentPageField('images', images);
  };
        ...
        <div className="relative w-full max-w-6xl flex-1 flex flex-col lg:flex-row gap-4 z-10 pb-8">
          ...
          <div className="flex-1 flex flex-col lg:flex-row gap-4">
            ...
            <PageEditorPanel
              ...
              onImagesChange={updateCurrentPageImages}
            />
          </div>
        </div>
      )}
